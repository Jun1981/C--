...
void init()
{
	...
	teki[0][0][0].count = 40;//敵が何ループごとに動くか
	...
	tama.sp = 4;//弾スピード
		...
		teki[0][0][0].x = (lx - 1) + lx*i;//緑の敵のｘ座標
		...
		teki[0][0][0].sp = lx / 2;//敵の一回の移動距離


		teki[0][0][0].dead = 0;//敵に弾が当たったかどうか（１で当たり）
		...
		teki[0][0][0].haji = 0;//敵が端に着いたかどうか（１で着いた）
